	...
	public function index()
	{
		...
		*/
				$productos = DB::table('pedidosproductos')						
						->select('productos.id','productos.producto','productos.descripcion',  	DB::raw('sum(pedidosproductos.num_productos) as total'))
						->Join('productos','pedidosproductos.producto_id','=','productos.id')
						//->sum('pedidosproductos.num_productos');
						//->sum('pedidosproductos.num_productos');
						->orderBy(DB::raw('sum(\'pedidosproductos.num_productos\')'))
				...
				$clientes = DB::table('pedidos')						
						->select('clientes.id','clientes.nombres','clientes.apellidos',  	DB::raw('sum(pedidos.cliente_id) as total'))
						->Join('clientes','pedidos.cliente_id','=','clientes.id')
						//->sum('pedidosproductos.num_productos');
						//->sum('pedidosproductos.num_productos');
						->orderBy(DB::raw('sum(\'pedidos.cliente_id\')'))
						->groupBy('clientes.id')
						 ->take(10)
...
$clientes2 = $clientes; 
		...
		$columnas = array('producto'=>'Producto','descripcion'=>'Descripcion','total'=>'TOTAL VENDIDO');
		$data = array('productos' => $productos, 'columnas' => $columnas );
		return View::make('admin/reporteIndex')->with('productos', $productos2)->with('clientes', $clientes2);
	}
